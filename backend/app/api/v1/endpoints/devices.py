...
@router.post("/interferer", status_code=status.HTTP_201_CREATED, response_model=DeviceSchema)
async def create_new_interferer(
    *,
    ...
    # 檢查名稱是否已存在
    existing_device = await crud_device.get_device_by_name(db=session, name=interferer_in.name)
    if existing_device:
    ...
    try:
        created_device = await crud_device.create_interferer(db=session, obj_in=interferer_in)
        # 確保返回一個有效的 Device 對象
        interferer_db = await crud_device.get_device(db=session, device_id=created_device.id)
...
async def read_devices(
    ...
    transmitter_type: Optional[TransmitterType] = Query(None, description="Filter by transmitter type (only for TRANSMITTER devices)"),
) -> Any:
    """
    獲取設備列表，可選按設備類型和發射器類型過濾。
    """
    logger.info(f"API: Received request to read devices (skip={skip}, limit={limit}, type={device_type}, tx_type={transmitter_type})")
    
    if transmitter_type and (device_type is None or device_type != DeviceType.TRANSMITTER):
        # 如果指定了發射器類型但設備類型不是發射器，則設置設備類型為發射器
        device_type = DeviceType.TRANSMITTER
        logger.info(f"Setting device_type to TRANSMITTER as transmitter_type was specified")
    ...
    if device_type == DeviceType.TRANSMITTER and transmitter_type:
        ...
        # 干擾源 - 使用專門的干擾源查詢
        devices = await crud_device.get_multi_interferers(db=session, skip=skip, limit=limit)
    else:
        # 一般設備查詢
...
async def read_interferers(
    ...
    interferers = await crud_device.get_multi_interferers(db=session, skip=skip, limit=limit)
...
async def read_device_by_id(
    device_id: int,
    session: AsyncSession = Depends(get_session),
    as_interferer: bool = Query(False, description="Try to get the device as an interferer"),
) -> Any:
    """
    根據 ID 獲取單個設備。
    ...
    但如果不是干擾源，則將返回一般設備數據。
    """
    logger.info(f"API: Received request to read device with ID: {device_id}, as_interferer={as_interferer}")
...
async def read_interferer_by_id(
    ...
    interferer = await crud_device.get_interferer(db=session, interferer_id=interferer_id)
    if not interferer:
...
async def update_existing_interferer(
    *,
    ...
    db_interferer = await crud_device.get_interferer(db=session, interferer_id=interferer_id)
...
async def delete_interferer_by_id(
    *,
    ...
    deleted_interferer = await crud_device.remove_interferer(db=session, interferer_id=interferer_id)
